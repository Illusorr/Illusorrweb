...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.addEventListener('click', (e) => {
        ...
        if (act === 'replace') {
          ...
          // Announce for any host picker that wants it, but always open the
          // local browse dialog — outside the host runtime the event alone
    ...
    async _ingest(file) {
      this._setError(null);
      // Validate by decodability, not by MIME string: pickers hand back empty
      // or vendor types (HEIC from phones, image/jpg, or no type at all) that
      // the browser can still decode. Only a failed decode is a real reject.
      if (!file) { this._setError('Choose an image file.'); return; }
      if (!/^image\//.test(file.type || '') &&
          !/\.(png|jpe?g|webp|avif|gif|heic|heif|bmp|tiff?)$/i.test(file.name || '')) {
        this._setError('That file is not an image.');
        return;
      }
